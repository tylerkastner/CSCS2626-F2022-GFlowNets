...
grid_height = 8
env = HyperGrid(ndim=2, height=grid_height, R0=0.01)  # Grid of size 8x8x8x8
env.render_gt()
...
visited_terminating_states = (
    env.States.from_batch_shape((0,)) if not resample_for_validation else None
)
if use_replay_buffer > 0:
    replay_buffer = ReplayBuffer(env, loss_fn, capacity=replay_buffer_size)
...
for i in tqdm.trange(1000):
    ...
    if i % validation_interval == 0:
        validation_info, final_states_dist_pmf = validate(env, parametrization, n_validation_samples, visited_terminating_states, return_terminating_distribution=True)
        to_log.update(validation_info)
        tqdm.tqdm.write(f"Iteration: {i}: {to_log}")

        plt.matshow(final_states_dist_pmf.reshape(grid_height, grid_height).T)
        plt.show()
